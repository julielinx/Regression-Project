...
Of course, to determine if there is a better fit model we would need to account for how all the vairables interacted with each other as well. The first step is the create a model that reflects all variables and all variable interactions.
...
We also ran the backward stepwise regression function and examined the results to compare them to our manual version. Interestingly, it came up with slightly different variables. The first noticable change is that the function kept Taxes instead of SqFt. Additionally, it kept Age:OperExp and OperExp:VacRate where our manual model recommended removing them.
...
The result was that the model created by the function had a slightly better adjusted R^2 of 0.844 compared to 0.8358, but a significally worse F-statistic of 54.55 compared to 63.99. As such, we moved forward with our analysis using the model created using the manual method.
```{r}
...
The models showed generally decreasing p-values, but these reductions were marginal. The reduction was more than made up for by the increases in the F-statistic - a difference of 24.54. The updated model has also given us a tighter range. It decrases our initial range of \$4,074.80 down to an updated range of \$3,282.28. A difference of nearly $800.

An analysis of the residuals shows that there is still no discernable pattern and even shows slightly less clumping than the model created from just single variables. The analysis of variance on both the single variable and manual stepwise models shows good significance. The significance for the OperExp:W2MiDT variable appears to fall outside the range of significance although it was within bounds when running the analysis from the coefficents. The fitted plot shows tighter grouping around the line, but still quite a bit of variance from the prediction.
```{r}
full.df <- fortify(df_model)
ggplot(full.df, aes(x=.fitted, y=.resid)) +
  geom_point() +
...
Lastly we use our model to determine a range for what we can charge in rent for our building. For this we used the model created by the manual stepwise regression. The model the vairables available indicates we chould charge between \$13,984.76 - \$17,267.04 per month to rent out the building. Taking into account operating expenses and taxes, we pay \$13,540 to run the building. There may be additional expenses we should factor in, such as maintenance expenses for upkeep and repair. However, based on the current information, we could easily rent the building for over \$13,540.
```{r}

...
Of course, to determine if there is a better fit model we would need to account for how all the variables interacted with each other as well. The first step is the create a model that reflects all variables and all variable interactions.
...
An analysis of the residuals shows that there is still no discernible pattern and even shows slightly less clumping than the model created from just single variables. The analysis of variance on both the single variable and manual stepwise models shows good significance. The significance for the OperExp:W2MiDT variable appears to fall outside the range of significance although it was within bounds when running the analysis from the coefficients. The fitted plot shows tighter grouping around the line, but still quite a bit of variance from the prediction.
```{r}
full.df <- fortify(df_model)
ggplot(full.df, aes(x=.fitted, y=.resid)) +
  geom_point() +
...
Lastly we use our model to determine a range for what we can charge in rent for our building. For this we used the model created by the manual stepwise regression. The model indicates we could charge between \$13,984.76 - \$17,267.04 per month to rent out the building. Taking into account that our break even point to run the building is  \$13,540, our model indicates we should rent the space. There may be additional expenses we should factor in, such as maintenance expenses for upkeep and repair. However, based on the current information, we could easily rent the building for over \$13,540.
```{r}
...
Our final model explains around 83% of the variability in the data we collected on 100 properties. This should give us a good enough idea on what to charge to rent out the building in order to decide to rent instead of sell. However, there may be other variables we need to take into account in order to establish a better range. One variable that comes to mind is the neighborhood. For example, the desirability of a building in the Gulch is different than in Germantown. The facilities available in the building could also effect the amount charged for rent. For example, whether or not the building be used as a restaurant, store, or solely office space.

In being a statistical model, it would probably need to be explained to a business audience. But the included graphics should serve to illustrate the point.